...
func TestAddProposalIDToFinish(t *testing.T) {
	...
	tests := map[string]struct {
		...
		"OK": {
			...
			caminoState: &caminoState{
				caminoDiff: &caminoDiff{
					addedProposalIDsToFinish: []ids.ID{proposalID1, proposalID2},
				},
			},
			expectedCaminoState: &caminoState{
				caminoDiff: &caminoDiff{
					addedProposalIDsToFinish: []ids.ID{proposalID1, proposalID2, proposalID3},
				},
			},
		},
	}
...
func TestGetProposalIDsToFinish(t *testing.T) {
	...
	tests := map[string]struct {
		...
		"OK": {
			caminoState: &caminoState{
				caminoDiff: &caminoDiff{
					addedProposalIDsToFinish: []ids.ID{proposalID3, proposalID4},
				},
				proposalIDsToFinish: []ids.ID{proposalID1, proposalID2},
			},
			expectedCaminoState: &caminoState{
			...
			expectedProposalIDsToFinish: []ids.ID{proposalID1, proposalID2, proposalID3, proposalID4},
		},
	}
	for name, tt := range tests {
		t.Run(name, func(t *testing.T) {
...
func TestWriteProposals(t *testing.T) {
	...
	tests := map[string]struct {
		...
		"OK: add proposals-to-finish": {
			caminoState: func(c *gomock.Controller) *caminoState {
				proposalIDsToFinishList := linkeddb.NewMockLinkedDB(c)
				proposalIDsToFinishList.EXPECT().Put(proposalID1[:], nil).Return(nil)
				proposalIDsToFinishList.EXPECT().Put(proposalID2[:], nil).Return(nil)
				...
				return &caminoState{
					proposalIDsToFinishList: proposalIDsToFinishList,
			...
			expectedCaminoState: func(actualCaminoState *caminoState) *caminoState {
				return &caminoState{
					proposalIDsToFinishList: actualCaminoState.proposalIDsToFinishList,
					proposalIDsByEndtimeDB:  actualCaminoState.proposalIDsByEndtimeDB,
					caminoDiff:              &caminoDiff{},
					proposalIDsToFinish:     []ids.ID{proposalID1, proposalID2},
				}
	...
	tests := map[string]struct {
		...
		"OK": {
			caminoState: func(c *gomock.Controller) *caminoState {
				...
				proposalIDsToFinishList := linkeddb.NewMockLinkedDB(c)
				proposalIDsToFinishList.EXPECT().NewIterator().Return(proposalsToFinishIterator)
...
// Test that proposalIDsToFinish order is maintened with write/load
func TestProposalIDsToFinishOrder(t *testing.T) {
	proposalID1 := ids.ID{1}
	proposalID2 := ids.ID{2}
	proposalID3 := ids.ID{3}
	...
	caminoState, ok := state.caminoState.(*caminoState)
	require.True(t, ok)
	require.NoError(t, database.PutBool(caminoState.caminoDB, nodeSignatureKey, caminoState.verifyNodeSignature))
	require.NoError(t, database.PutBool(caminoState.caminoDB, depositBondModeKey, caminoState.lockModeBondDeposit))
	require.NoError(t, database.PutTimestamp(state.singletonDB, timestampKey, state.persistedTimestamp))
	require.NoError(t, database.PutUInt64(state.singletonDB, currentSupplyKey, state.persistedCurrentSupply))
	require.NoError(t, database.PutID(state.singletonDB, lastAcceptedKey, state.persistedLastAccepted))
	state.AddProposalIDToFinish(proposalID1)
	state.AddProposalIDToFinish(proposalID2)
	state.AddProposalIDToFinish(proposalID3)
	require.NoError(t, state.Commit())
	proposalIDsToFinish, err := state.GetProposalIDsToFinish()
	require.NoError(t, err)
	require.Equal(t, []ids.ID{proposalID1, proposalID2, proposalID3}, proposalIDsToFinish)

	// adding new proposalIDs
	state.AddProposalIDToFinish(proposalID4)
	state.AddProposalIDToFinish(proposalID5)
	state.AddProposalIDToFinish(proposalID6)
	proposalIDsToFinish, err = state.GetProposalIDsToFinish()
	require.NoError(t, err)
	require.Equal(t, []ids.ID{proposalID1, proposalID2, proposalID3, proposalID4, proposalID5, proposalID6}, proposalIDsToFinish)

	// removing some proposalIDs from initial and from newly added
	state.RemoveProposalIDToFinish(proposalID2)
	state.RemoveProposalIDToFinish(proposalID4)
	state.RemoveProposalIDToFinish(proposalID6)
	proposalIDsToFinish, err = state.GetProposalIDsToFinish()
	require.NoError(t, err)
	require.Equal(t, []ids.ID{proposalID1, proposalID3, proposalID5}, proposalIDsToFinish)
