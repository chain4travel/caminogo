...
func (cs *caminoState) AddProposalIDToFinish(proposalID ids.ID) {
	cs.addedProposalIDsToFinish = append(cs.addedProposalIDsToFinish, proposalID)
}

func (cs *caminoState) GetProposalIDsToFinish() ([]ids.ID, error) {
	proposalIDsToFinish := make([]ids.ID, 0, len(cs.proposalIDsToFinish)+len(cs.addedProposalIDsToFinish)-len(cs.removedProposalIDsToFinish))
	for _, proposalID := range cs.proposalIDsToFinish {
		if _, ok := cs.removedProposalIDsToFinish[proposalID]; !ok {
			proposalIDsToFinish = append(proposalIDsToFinish, proposalID)
		}
	}
	for _, proposalID := range cs.addedProposalIDsToFinish {
		if _, ok := cs.removedProposalIDsToFinish[proposalID]; !ok {
			proposalIDsToFinish = append(proposalIDsToFinish, proposalID)
		}
	}
...
func (cs *caminoState) RemoveProposalIDToFinish(proposalID ids.ID) {
	cs.removedProposalIDsToFinish[proposalID] = struct{}{}
}
...
func (cs *caminoState) writeProposals() error {
	...
	for _, proposalID := range cs.addedProposalIDsToFinish {
		if _, ok := cs.removedProposalIDsToFinish[proposalID]; !ok {
			if err := cs.proposalIDsToFinishList.Put(proposalID[:], nil); err != nil {
				return err
			}
			proposalIDsToFinish = append(proposalIDsToFinish, proposalID)
		}
	...
	for proposalID := range cs.removedProposalIDsToFinish {
		if err := cs.proposalIDsToFinishList.Delete(proposalID[:]); err != nil {
			return err
		}
		delete(cs.removedProposalIDsToFinish, proposalID)
	}

	cs.addedProposalIDsToFinish = nil
...
func (cs *caminoState) loadProposals() error {
	...
	proposalsToFinishIterator := cs.proposalIDsToFinishList.NewIterator()
