...
var (
	...
	_ UnsignedProposalTx = &UnsignedDaoAddVoteTx{}
)

// UnsignedDaoAddVoteTx is an unsigned daoVoteTx
type UnsignedDaoAddVoteTx struct {
...
// [UnsignedDaoAddVoteTx]. Also sets the [ctx] to the given [vm.ctx] so that
// the addresses can be json marshalled into human readable format
func (tx *UnsignedDaoAddVoteTx) InitCtx(ctx *snow.Context) {
	tx.BaseTx.InitCtx(ctx)
}

// SyntacticVerify returns nil if [tx] is valid
func (tx *UnsignedDaoAddVoteTx) SyntacticVerify(ctx *snow.Context) error {
	switch {
	case tx == nil:
		return errNilTx
...
func (tx *UnsignedDaoAddVoteTx) SemanticVerify(vm *VM, parentState MutableState, stx *Tx) error {
	_, _, err := tx.Execute(vm, parentState, stx)
	return err
}

// Execute this transaction.
func (tx *UnsignedDaoAddVoteTx) Execute(
	vm *VM,
	...
	for _, in := range tx.Ins {
		if in.AssetID() == vm.ctx.AVAXAssetID { // only allow to use avax to bond
			totalTransferable += in.In.Amount()
		}
	}

	if totalTransferable < vm.DaoConfig.VoteTxFee {
		return nil, nil, fmt.Errorf("provided ins do not cover for vote fee")
	}
	...
	if vm.bootstrapped.GetValue() {
		...
		case len(daoProposal.Votes()) >= int(daoProposal.DaoProposalTx().ProposalConfiguration.Thresh):
...
// should be inserted.
func (tx *UnsignedDaoAddVoteTx) InitiallyPrefersCommit(vm *VM) bool {
	return true
}

// TimedTx Interface
func (tx *UnsignedDaoAddVoteTx) StartTime() time.Time { return time.Unix(tx.Start, 0) }
func (tx *UnsignedDaoAddVoteTx) EndTime() time.Time   { return mockable.MaxTime }
func (tx *UnsignedDaoAddVoteTx) Weight() uint64       { return 0 }

// newDaoVoteTx returns a new UnsignedDaoAddVoteTx
