...
type MultisigAlias struct {
	...
	MemoBytes []byte        `serialize:"true" json:"memo"`
}

func NewMultisigAlias(txID ids.ID, addresses []ids.ShortID, threshold uint32, memo []byte) (MultisigAlias, error) {
...
func (ma *MultisigAlias) Verify(txID ids.ID) error {
	if len(ma.MemoBytes) > avax.MaxMemoSize {
		return fmt.Errorf("msig alias memo is larger (%d bytes) than max of %d bytes", len(ma.MemoBytes), avax.MaxMemoSize)
	}
...
func (b *Block) Txs() []*txs.Tx {
	txs := make([]*txs.Tx, 0, len(b.Validators)+len(b.Deposits))
