...
func TestProduceUTXOs(t *testing.T) {
	...
	tests := []struct {
		...
		generateWant         func(ins []*avax.TransferableInput, indexes []uint32, outs []*avax.TransferableOutput, producedUTXOs []*avax.UTXO) want
	}{
		{
			...
			generateWant: func(ins []*avax.TransferableInput, indexes []uint32, outs []*avax.TransferableOutput, producedUTXOs []*avax.UTXO) want {
				return want{
					utxoLockedState: map[ids.ID]utxoLockState{
						ins[indexes[0]].InputID():  {},                     // consumedUTXO
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			utxoLockedState, utxos := currentLockedState.ProduceUTXOsAndLockState(tt.args.inputs, tt.args.inputIndexes, tt.args.outputs, tt.args.txID)
			generatedWant := tt.generateWant(tt.args.inputs, tt.args.inputIndexes, tt.args.outputs, utxos)
