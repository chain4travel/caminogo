...
func (e *CaminoStandardTxExecutor) FinishProposalsTx(tx *txs.FinishProposalsTx) error {
	...
	lockTxIDs := append(tx.EarlyFinishedProposalIDs, tx.ExpiredFailedProposalIDs...) //nolint:gocritic
	...
	for _, proposalID := range tx.EarlyFinishedProposalIDs {
		...
		// ! @evlekht possible future critical issue: when adding new proposals,
		// ! 	we can't foresee if proposals will conflict at the execution time
		// ! suggestion: simply allow proposals to fail and finish them without execution if its the case
		// ! possible issue: if allow to fail, then it could be hard to distinct
		// ! 	case of corrupted state making proposal fail from conflicting proposal
	...
	for _, proposalID := range tx.ExpiredSuccessfulProposalIDs {
		...
		// ! @evlekht possible future critical issue: see ! comment above
		// try to execute proposal
		if err := proposal.Visit(e.proposalExecutor()); err != nil {
			return err
		}
