...
func TestCaminoLockedInsOrLockedOuts(t *testing.T) {
	...
	tests := map[string]struct {
		...
		"Locked out - LockModeBondDeposit: true": {
			outs: []*avax.TransferableOutput{
				generate.Out(test.AVAXAssetID, test.ValidatorWeight, outputOwners, ids.Empty, ids.GenerateTestID()),
			},
			ins:         []*avax.TransferableInput{},
		...
		"Locked in - LockModeBondDeposit: true": {
			outs: []*avax.TransferableOutput{},
			ins: []*avax.TransferableInput{
				generate.In(test.AVAXAssetID, test.ValidatorWeight, ids.GenerateTestID(), ids.Empty, sigIndices),
			},
		...
		"Stakeable out - LockModeBondDeposit: true": {
			outs: []*avax.TransferableOutput{
				generate.StakeableOut(test.AVAXAssetID, test.ValidatorWeight, uint64(test.MinStakingDuration), outputOwners),
			},
			ins:         []*avax.TransferableInput{},
			expectedErr: locked.ErrWrongOutType,
			caminoConfig: api.Camino{
		...
		"Stakeable in - LockModeBondDeposit: true": {
			outs: []*avax.TransferableOutput{},
			ins: []*avax.TransferableInput{
				generate.StakeableIn(test.AVAXAssetID, test.ValidatorWeight, uint64(test.MinStakingDuration), sigIndices),
			},
			expectedErr: locked.ErrWrongInType,
			caminoConfig: api.Camino{
				VerifyNodeSignature: true,
...
func TestCaminoAddSubnetValidatorTxNodeSig(t *testing.T) {
	...
	tests := map[string]struct {
		...
		"Happy path, LockModeBondDeposit false, VerifyNodeSignature true": {
			...
			nodeKey: nodeKey1,
			utxos: []*avax.UTXO{
				generate.UTXO(ids.ID{1}, test.AVAXAssetID, test.ValidatorWeight*2, outputOwners, ids.Empty, ids.Empty, true),
			},
			outs: []*avax.TransferableOutput{
				generate.Out(test.AVAXAssetID, test.ValidatorWeight-test.TxFee, outputOwners, ids.Empty, ids.Empty),
			},
			stakedOuts: []*avax.TransferableOutput{
				generate.StakeableOut(test.AVAXAssetID, test.ValidatorWeight, uint64(test.MinStakingDuration), outputOwners),
			},
...
func TestCaminoStandardTxExecutorCaminoRewardValidatorTx(t *testing.T) {
	caminoGenesisConf := api.Camino{
		VerifyNodeSignature: true,
		LockModeBondDeposit: true,
	}
	...
	nodeOwnerAddr := ids.ShortID{1, 1, 1}

	addValidatorTxID := ids.ID{1, 1}
	wrongAddValidatorTxID := ids.ID{2, 2}
	depositTxID := ids.ID{3, 3}

	_, bondOwnerAddr, bondOwner := generate.KeyAndOwner(t, test.Keys[0])
	_, depositBondOwnerAddr, depositBondOwner := generate.KeyAndOwner(t, test.Keys[1])

	bondedUTXO := generate.UTXO(ids.ID{3}, test.AVAXAssetID, 7, bondOwner, ids.Empty, addValidatorTxID, true)
	depositedBondedUTXO := generate.UTXO(ids.ID{4}, test.AVAXAssetID, 13, depositBondOwner, depositTxID, addValidatorTxID, true)

	ins := []*avax.TransferableInput{
		generate.InFromUTXO(t, bondedUTXO, []uint32{}, false),
		generate.InFromUTXO(t, depositedBondedUTXO, []uint32{}, false),
	}
	outs := []*avax.TransferableOutput{
		generate.Out(test.AVAXAssetID, 7, bondOwner, ids.Empty, ids.Empty),
		generate.Out(test.AVAXAssetID, 13, depositBondOwner, depositTxID, ids.Empty),
	}

	caminoRewardValidatorTx := &txs.CaminoRewardValidatorTx{
		RewardValidatorTx: txs.RewardValidatorTx{TxID: addValidatorTxID},
		Ins:               ins,
		Outs:              outs,
	}

	emptyState := func(_ *testing.T, ctrl *gomock.Controller, _ *config.Config, _ test.Phase, _ *txs.CaminoRewardValidatorTx, _ ids.ID) *state.MockDiff {
		s := state.NewMockDiff(ctrl)
		return s
	}

	tests := map[string]struct {
		onCommitState func(*testing.T, *gomock.Controller, *config.Config, test.Phase, *txs.CaminoRewardValidatorTx, ids.ID) *state.MockDiff
		onAbortState  func(*testing.T, *gomock.Controller, *config.Config, test.Phase, *txs.CaminoRewardValidatorTx, ids.ID) *state.MockDiff
		phase         test.Phase
		tx            *txs.CaminoRewardValidatorTx
		signers       [][]*secp256k1.PrivateKey
		expectedErr   error
	}{
		"Not zero credentials": {
			onCommitState: func(_ *testing.T, ctrl *gomock.Controller, _ *config.Config, _ test.Phase, _ *txs.CaminoRewardValidatorTx, _ ids.ID) *state.MockDiff {
				s := state.NewMockDiff(ctrl)
				s.EXPECT().CaminoConfig().Return(caminoConfig, nil)
				return s
			},
			onAbortState: emptyState,
			phase:        test.PhaseLast,
			tx:           caminoRewardValidatorTx,
			signers:      [][]*secp256k1.PrivateKey{{}},
			expectedErr:  errWrongCredentialsNumber,
		},
		"Invalid inputs (one excess)": {
			onCommitState: func(t *testing.T, ctrl *gomock.Controller, cfg *config.Config, phase test.Phase, tx *txs.CaminoRewardValidatorTx, _ ids.ID) *state.MockDiff {
				s := state.NewMockDiff(ctrl)
				s.EXPECT().CaminoConfig().Return(caminoConfig, nil)
				expect.Unlock(t, s,
					[]ids.ID{tx.TxID},
					[]ids.ShortID{bondOwnerAddr, depositBondOwnerAddr},
					[]*avax.UTXO{bondedUTXO, depositedBondedUTXO},
					locked.StateBonded,
				)
				return s
			},
			...
			tx: &txs.CaminoRewardValidatorTx{
				...
				Ins: append(
					ins,
					generate.In(test.AVAXAssetID, 3, ids.Empty, ids.Empty, []uint32{}), // excess input
				),
				Outs: outs,
			},
		...
		"Invalid inputs (wrong amount)": {
			...
			tx: &txs.CaminoRewardValidatorTx{
				...
				Ins: func() []*avax.TransferableInput {
					wrongIns := make([]*avax.TransferableInput, len(ins))
					copy(wrongIns, ins)
					wrongIns[0] = &avax.TransferableInput{
						UTXOID: ins[0].UTXOID,
						Asset:  ins[0].Asset,
						In: &locked.In{
							IDs: ins[0].In.(*locked.In).IDs,
							TransferableIn: &secp256k1fx.TransferInput{
								Amt: ins[0].In.Amount() + 1, // wrong amount
							},
						},
					}
					return wrongIns
		...
		"Invalid outputs (one excess)": {
			...
			tx: &txs.CaminoRewardValidatorTx{
				...
				Outs: append(
					outs,
					generate.Out(test.AVAXAssetID, 3, bondOwner, ids.Empty, ids.Empty), // excess output
				),
			},
			expectedErr: errInvalidSystemTxBody,
		},
		"Invalid outputs (wrong amount)": {
			...
			tx: &txs.CaminoRewardValidatorTx{
				...
				Outs: func() []*avax.TransferableOutput {
					wrongOuts := make([]*avax.TransferableOutput, len(outs))
					copy(wrongOuts, outs)
					wrongOuts[0] = &avax.TransferableOutput{
						Asset: outs[0].Asset,
						Out: &secp256k1fx.TransferOutput{
							Amt:          outs[0].Out.Amount() + 1, // wrong amount
							OutputOwners: bondOwner,
						},
					}
					return wrongOuts
				}(),
		...
		"Wrong staker": {
			onCommitState: func(t *testing.T, ctrl *gomock.Controller, cfg *config.Config, phase test.Phase, tx *txs.CaminoRewardValidatorTx, _ ids.ID) *state.MockDiff {
				stakerToRemove := &state.Staker{TxID: addValidatorTxID}
				...
				deferredStakerIterator := state.NewMockStakerIterator(ctrl)
				deferredStakerIterator.EXPECT().Next().Return(false)
				...
				s.EXPECT().GetDeferredStakerIterator().Return(deferredStakerIterator, nil)
				return s
			},
			onAbortState: emptyState,
			phase:        test.PhaseLast,
			tx: &txs.CaminoRewardValidatorTx{
				RewardValidatorTx: txs.RewardValidatorTx{TxID: wrongAddValidatorTxID}, // not next current/deferred staker addValidatorTx
				Ins:               ins,
				Outs:              outs,
			},
			expectedErr: errRemoveWrongValidator,
		},
		"Remove before end time": {
			onCommitState: func(t *testing.T, ctrl *gomock.Controller, cfg *config.Config, phase test.Phase, tx *txs.CaminoRewardValidatorTx, _ ids.ID) *state.MockDiff {
				chainTime := test.PhaseTime(t, phase, cfg)
				stakerToRemove := &state.Staker{
					TxID:    tx.TxID,
					EndTime: chainTime.Add(1), // too early for removal
				}
		...
		"OK": {
			onCommitState: func(t *testing.T, ctrl *gomock.Controller, cfg *config.Config, phase test.Phase, tx *txs.CaminoRewardValidatorTx, txID ids.ID) *state.MockDiff {
				chainTime := test.PhaseTime(t, phase, cfg)
				stakerToRemove := &state.Staker{
					TxID:    tx.TxID,
					EndTime: chainTime,
				...
				s.EXPECT().GetDeferredStakerIterator().Return(deferredStakerIterator, nil)
				s.EXPECT().GetTimestamp().Return(chainTime)
				s.EXPECT().GetTx(tx.TxID).Return(&txs.Tx{Unsigned: &txs.CaminoAddValidatorTx{}}, status.Committed, nil)
				s.EXPECT().DeleteCurrentValidator(stakerToRemove)
				expect.ConsumeUTXOs(t, s, tx.Ins)
				expect.ProduceUTXOs(t, s, tx.Outs, txID, 0)
				return s
			},
			onAbortState: func(_ *testing.T, ctrl *gomock.Controller, cfg *config.Config, phase test.Phase, tx *txs.CaminoRewardValidatorTx, txID ids.ID) *state.MockDiff {
				chainTime := test.PhaseTime(t, phase, cfg)
				stakerToRemove := &state.Staker{
					TxID:    tx.TxID,
					EndTime: chainTime,
		...
		"OK: deferred": {
			onCommitState: func(t *testing.T, ctrl *gomock.Controller, cfg *config.Config, phase test.Phase, tx *txs.CaminoRewardValidatorTx, txID ids.ID) *state.MockDiff {
				...
				deferredStakerIterator := state.NewMockStakerIterator(ctrl)
				deferredStakerIterator.EXPECT().Next().Return(true)
				deferredStakerIterator.EXPECT().Value().Return(stakerToRemove)
				deferredStakerIterator.EXPECT().Release()
				...
				s.EXPECT().GetTimestamp().Return(chainTime)
				s.EXPECT().GetTx(tx.TxID).Return(&txs.Tx{Unsigned: &txs.CaminoAddValidatorTx{}}, status.Committed, nil)
				s.EXPECT().DeleteDeferredValidator(stakerToRemove)

				s.EXPECT().GetShortIDLink(ids.ShortID(stakerToRemove.NodeID), state.ShortLinkKeyRegisterNode).Return(nodeOwnerAddr, nil)
				nodeOwnerAddrState := as.AddressStateNodeDeferred | as.AddressStateConsortium
				s.EXPECT().GetAddressStates(nodeOwnerAddr).Return(nodeOwnerAddrState, nil)
				s.EXPECT().SetAddressStates(nodeOwnerAddr, nodeOwnerAddrState & ^as.AddressStateNodeDeferred)
			...
			onAbortState: func(_ *testing.T, ctrl *gomock.Controller, cfg *config.Config, phase test.Phase, tx *txs.CaminoRewardValidatorTx, txID ids.ID) *state.MockDiff {
				...
				s := state.NewMockDiff(ctrl)
				s.EXPECT().DeleteDeferredValidator(stakerToRemove)
				s.EXPECT().SetAddressStates(nodeOwnerAddr, as.AddressStateConsortium)
				expect.ConsumeUTXOs(t, s, tx.Ins)
				expect.ProduceUTXOs(t, s, tx.Outs, txID, 0)
				return s
			},
	...
	for name, tt := range tests {
		t.Run(name, func(t *testing.T) {
			...
			txExecutor := CaminoProposalTxExecutor{
				ProposalTxExecutor{
					OnCommitState: tt.onCommitState(t, ctrl, backend.Config, tt.phase, tt.tx, tx.ID()),
					OnAbortState:  tt.onAbortState(t, ctrl, backend.Config, tt.phase, tt.tx, tx.ID()),
					Backend:       backend,
					Tx:            tx,
				},
			}
...
func TestCaminoStandardTxExecutorAddressStateTx(t *testing.T) {
	...
	feeUTXO := generate.UTXO(ids.ID{1}, test.AVAXAssetID, defaultTxFee, feeOwner, ids.Empty, ids.Empty, false)
	halfFeeUTXO1 := generate.UTXO(ids.ID{2}, test.AVAXAssetID, defaultTxFee/2, feeOwner, ids.Empty, ids.Empty, false)
	halfFeeUTXO2 := generate.UTXO(ids.ID{3}, test.AVAXAssetID, defaultTxFee-defaultTxFee/2, executorOwner, ids.Empty, ids.Empty, false)
	msigFeeUTXO := generate.UTXO(ids.ID{4}, test.AVAXAssetID, defaultTxFee, msigAliasOwner, ids.Empty, ids.Empty, false)
...
func TestCaminoStandardTxExecutorDepositTx(t *testing.T) {
	...
	feeUTXO := generate.UTXO(ids.ID{1}, test.AVAXAssetID, test.TxFee, feeOwner, ids.Empty, ids.Empty, true)
	doubleFeeUTXO := generate.UTXO(ids.ID{1}, test.AVAXAssetID, test.TxFee*2, feeOwner, ids.Empty, ids.Empty, true)
	unlockedUTXO1 := generate.UTXO(ids.ID{2}, test.AVAXAssetID, offer.MinAmount, utxoOwner, ids.Empty, ids.Empty, true)
	unlockedUTXO2 := generate.UTXO(ids.ID{3}, test.AVAXAssetID, offerWithMaxAmount.RemainingAmount(), utxoOwner, ids.Empty, ids.Empty, true)
	unlockedUTXO3 := generate.UTXO(ids.ID{4}, test.AVAXAssetID, offerWithMaxRewardAmount.MaxRemainingAmountByReward(), utxoOwner, ids.Empty, ids.Empty, true)
	bondedUTXOWithMinAmount := generate.UTXO(ids.ID{4}, test.AVAXAssetID, offer.MinAmount, utxoOwner, ids.Empty, ids.ID{100}, true)
			...
			utx: func() *txs.DepositTx {
				return &txs.DepositTx{
					BaseTx: txs.BaseTx{BaseTx: avax.BaseTx{
						...
						BlockchainID: constants.PlatformChainID,
						Ins: []*avax.TransferableInput{
							generate.StakeableIn(test.AVAXAssetID, test.PreFundedBalance, uint64(test.MinStakingDuration), []uint32{0}),
						},
					}},
					RewardsOwner: &secp256k1fx.OutputOwners{},
				}
			...
			utx: func() *txs.DepositTx {
				return &txs.DepositTx{
					BaseTx: txs.BaseTx{BaseTx: avax.BaseTx{
						...
						BlockchainID: constants.PlatformChainID,
						Outs: []*avax.TransferableOutput{
							generate.StakeableOut(test.AVAXAssetID, test.PreFundedBalance, uint64(test.MinStakingDuration), utxoOwner),
						},
					}},
					RewardsOwner: &secp256k1fx.OutputOwners{},
				}
			...
			utx: func() *txs.DepositTx {
				return &txs.DepositTx{
					BaseTx: txs.BaseTx{BaseTx: avax.BaseTx{
						...
						BlockchainID: constants.PlatformChainID,
						Outs: []*avax.TransferableOutput{
							generate.Out(test.AVAXAssetID, offer.MinAmount-1, utxoOwner, locked.ThisTxID, ids.Empty),
						},
					}},
					DepositOfferID:  offer.ID,
			...
			utx: func() *txs.DepositTx {
				...
				return &txs.DepositTx{
					BaseTx: txs.BaseTx{BaseTx: avax.BaseTx{
						...
						BlockchainID: constants.PlatformChainID,
						Outs: []*avax.TransferableOutput{
							generate.Out(test.AVAXAssetID, amt, utxoOwner, locked.ThisTxID, ids.Empty),
						},
					}},
			...
			utx: func() *txs.DepositTx {
				return &txs.DepositTx{
					BaseTx: txs.BaseTx{BaseTx: avax.BaseTx{
						...
						Outs: []*avax.TransferableOutput{
							generate.Out(test.AVAXAssetID, offer.MinAmount, utxoOwner, locked.ThisTxID, ids.Empty),
						},
					}},
					DepositOfferID:  offer.ID,
					DepositDuration: offer.MinDuration,
			...
			utx: func() *txs.DepositTx {
				return &txs.DepositTx{
					BaseTx: txs.BaseTx{BaseTx: avax.BaseTx{
						...
						Outs: []*avax.TransferableOutput{
							generate.Out(test.AVAXAssetID, test.TxFee, newUTXOOwner, ids.Empty, ids.Empty),
							generate.Out(test.AVAXAssetID, offer.MinAmount, utxoOwner, locked.ThisTxID, ids.Empty),
						},
					}},
					DepositOfferID:  offer.ID,
			...
			utx: func() *txs.DepositTx {
				return &txs.DepositTx{
					BaseTx: txs.BaseTx{BaseTx: avax.BaseTx{
						...
						Outs: []*avax.TransferableOutput{
							generate.Out(test.AVAXAssetID, offer.MinAmount, utxoOwner, locked.ThisTxID, ids.ID{100}),
						},
					}},
					DepositOfferID:  offer.ID,
					DepositDuration: offer.MinDuration,
			...
			utx: func() *txs.DepositTx {
				return &txs.DepositTx{
					BaseTx: txs.BaseTx{BaseTx: avax.BaseTx{
						...
						Outs: []*avax.TransferableOutput{
							generate.Out(test.AVAXAssetID, offer.MinAmount, newUTXOOwner, locked.ThisTxID, ids.Empty),
						},
					}},
					DepositOfferID:  offer.ID,
					DepositDuration: offer.MinDuration,
...
func TestCaminoStandardTxExecutorUnlockDepositTx(t *testing.T) {
	...
	depositWithRewardTxID1 := ids.ID{2}
	...
	depositOfferWithReward := &deposits.Offer{
		ID:                    ids.ID{5},
		MinAmount:             1,
		MinDuration:           60,
		MaxDuration:           60,
	...
	depositAmount1 := uint64(10000)
	depositAmount2 := uint64(20000)

	feeUTXO := generate.UTXO(ids.ID{6}, test.AVAXAssetID, test.TxFee, feeOwner, ids.Empty, ids.Empty, true)
	moreFeeUTXO := generate.UTXO(ids.ID{7}, test.AVAXAssetID, test.TxFee*10, feeOwner, ids.Empty, ids.Empty, true)
	lessFeeUTXO := generate.UTXO(ids.ID{8}, test.AVAXAssetID, 1, feeOwner, ids.Empty, ids.Empty, true)
	deposit1UTXO := generate.UTXO(ids.ID{9}, test.AVAXAssetID, depositAmount1, owner1, depositTxID1, ids.Empty, true)
	deposit2UTXO := generate.UTXO(ids.ID{10}, test.AVAXAssetID, depositAmount2, owner1, depositTxID2, ids.Empty, true)
	deposit1WithRewardUTXO := generate.UTXO(ids.ID{11}, test.AVAXAssetID, depositAmount1, owner1, depositWithRewardTxID1, ids.Empty, true)
	deposit1UTXOLargerTxID := generate.UTXO(ids.ID{12}, test.AVAXAssetID, depositAmount1, owner1, depositTxID1, ids.Empty, true)
	unlockedUTXOWithLargerTxID := generate.UTXO(ids.ID{13}, test.AVAXAssetID, 1, owner1, ids.Empty, ids.Empty, true)

	tests := map[string]struct {
		state       func(*testing.T, *gomock.Controller, test.Phase, *config.Config, *txs.UnlockDepositTx, ids.ID) *state.MockDiff
		utx         func(test.Phase, *config.Config) *txs.UnlockDepositTx
		signers     [][]*secp256k1.PrivateKey
		phase       test.Phase
		expectedErr error
	}{
		"Before Cairo/Wrong lockModeBondDeposit flag": {
			state: func(_ *testing.T, c *gomock.Controller, _ test.Phase, _ *config.Config, utx *txs.UnlockDepositTx, txID ids.ID) *state.MockDiff {
				s := state.NewMockDiff(c)
				s.EXPECT().CaminoConfig().Return(&state.CaminoConfig{LockModeBondDeposit: false}, nil)
				return s
			},
		...
		"Before Cairo/Unlock before deposit's unlock period": {
			state: func(t *testing.T, c *gomock.Controller, phase test.Phase, cfg *config.Config, utx *txs.UnlockDepositTx, txID ids.ID) *state.MockDiff {
				s := state.NewMockDiff(c)
				s.EXPECT().CaminoConfig().Return(&state.CaminoConfig{LockModeBondDeposit: true}, nil)
				phaseTime := test.PhaseTime(t, phase, cfg)
				deposit := &deposits.Deposit{
					Duration:       offer.MinDuration,
					Start:          uint64(phaseTime.Unix()),
					Amount:         depositAmount1,
					DepositOfferID: offer.ID,
				}
				s.EXPECT().GetTimestamp().Return(deposit.StartUnlockTime(offer).Add(-time.Second)).Times(2)
				s.EXPECT().GetBaseFee().Return(test.TxFee, nil)
		...
		"Before Cairo/Unlock expired deposit, tx has unlocked input before deposited input": {
			state: func(t *testing.T, c *gomock.Controller, phase test.Phase, cfg *config.Config, utx *txs.UnlockDepositTx, txID ids.ID) *state.MockDiff {
				s := state.NewMockDiff(c)
				s.EXPECT().CaminoConfig().Return(&state.CaminoConfig{LockModeBondDeposit: true}, nil)
				phaseTime := test.PhaseTime(t, phase, cfg)
				deposit := &deposits.Deposit{Duration: offer.MinDuration, Start: uint64(phaseTime.Unix())}
				s.EXPECT().GetTimestamp().Return(deposit.EndTime()).Times(2)
		...
		"Before Cairo/Unlock expired deposit, tx has unlocked input after deposited input": {
			state: func(t *testing.T, c *gomock.Controller, phase test.Phase, cfg *config.Config, utx *txs.UnlockDepositTx, txID ids.ID) *state.MockDiff {
				s := state.NewMockDiff(c)
				s.EXPECT().CaminoConfig().Return(&state.CaminoConfig{LockModeBondDeposit: true}, nil)
				phaseTime := test.PhaseTime(t, phase, cfg)
		...
		"Before Cairo/Unlock active and expired deposits": {
			state: func(t *testing.T, c *gomock.Controller, phase test.Phase, cfg *config.Config, utx *txs.UnlockDepositTx, txID ids.ID) *state.MockDiff {
				s := state.NewMockDiff(c)
				s.EXPECT().CaminoConfig().Return(&state.CaminoConfig{LockModeBondDeposit: true}, nil)
				phaseTime := test.PhaseTime(t, phase, cfg)
				deposit1 := &deposits.Deposit{Duration: offer.MinDuration, Start: uint64(phaseTime.Unix())}
				deposit2 := &deposits.Deposit{Duration: offer.MaxDuration, Start: uint64(phaseTime.Unix())}
				s.EXPECT().GetTimestamp().Return(deposit1.EndTime()).Times(2)
		...
		"Before Cairo/Unlock not full amount, deposit expired": {
			state: func(t *testing.T, c *gomock.Controller, phase test.Phase, cfg *config.Config, utx *txs.UnlockDepositTx, txID ids.ID) *state.MockDiff {
				s := state.NewMockDiff(c)
				s.EXPECT().CaminoConfig().Return(&state.CaminoConfig{LockModeBondDeposit: true}, nil)
				phaseTime := test.PhaseTime(t, phase, cfg)
		...
		"Before Cairo/Unlock more, than available": {
			state: func(t *testing.T, c *gomock.Controller, phase test.Phase, cfg *config.Config, utx *txs.UnlockDepositTx, txID ids.ID) *state.MockDiff {
				s := state.NewMockDiff(c)
				s.EXPECT().CaminoConfig().Return(&state.CaminoConfig{LockModeBondDeposit: true}, nil)
				phaseTime := test.PhaseTime(t, phase, cfg)
				...
				depositHalfUnlockTime := deposit.StartUnlockTime(offer).Add(offer.UnlockPeriodDurationNano() / 2)
		...
		"Before Cairo/Burned tokens, while unlocking expired deposits": {
			state: func(t *testing.T, c *gomock.Controller, phase test.Phase, cfg *config.Config, utx *txs.UnlockDepositTx, txID ids.ID) *state.MockDiff {
				s := state.NewMockDiff(c)
				s.EXPECT().CaminoConfig().Return(&state.CaminoConfig{LockModeBondDeposit: true}, nil)
				phaseTime := test.PhaseTime(t, phase, cfg)
		...
		"Before Cairo/Only burn fee, nothing unlocked": {
			state: func(t *testing.T, c *gomock.Controller, phase test.Phase, cfg *config.Config, utx *txs.UnlockDepositTx, txID ids.ID) *state.MockDiff {
				s := state.NewMockDiff(c)
				s.EXPECT().CaminoConfig().Return(&state.CaminoConfig{LockModeBondDeposit: true}, nil)
				phaseTime := test.PhaseTime(t, phase, cfg)
		...
		"Before Cairo/OK: no deposited input": { // we have such txs in network already
			state: func(t *testing.T, c *gomock.Controller, phase test.Phase, cfg *config.Config, utx *txs.UnlockDepositTx, txID ids.ID) *state.MockDiff {
				...
				// checks
				s.EXPECT().CaminoConfig().Return(&state.CaminoConfig{LockModeBondDeposit: true}, nil)
				s.EXPECT().GetTimestamp().Return(test.PhaseTime(t, phase, cfg)).Times(2)
				s.EXPECT().GetBaseFee().Return(test.TxFee, nil)
				expect.VerifyUnlockDeposit(t, s, utx.Ins,
					[]*avax.UTXO{moreFeeUTXO},
					[]ids.ShortID{
						feeOwnerAddr, // consumed unlocked
						feeOwnerAddr, // produced unlocked
					}, nil)
		...
		"Before Cairo/OK: unlock full amount, expired deposit with unclaimed reward": {
			state: func(t *testing.T, c *gomock.Controller, phase test.Phase, cfg *config.Config, utx *txs.UnlockDepositTx, txID ids.ID) *state.MockDiff {
				s := state.NewMockDiff(c)
				// checks
				s.EXPECT().CaminoConfig().Return(&state.CaminoConfig{LockModeBondDeposit: true}, nil)
				...
				s.EXPECT().GetDeposit(depositWithRewardTxID1).Return(deposit, nil).Times(2)
				s.EXPECT().GetDepositOffer(depositOfferWithReward.ID).Return(depositOfferWithReward, nil)
				s.EXPECT().GetClaimable(owner1ID).Return(&state.Claimable{Owner: &owner1}, nil)
				s.EXPECT().SetClaimable(owner1ID, &state.Claimable{
					Owner:                &owner1,
					ExpiredDepositReward: deposit.RemainingReward(depositOfferWithReward),
				})
				s.EXPECT().RemoveDeposit(depositWithRewardTxID1, deposit)
				// state update: ins/outs/utxos
				expect.ConsumeUTXOs(t, s, utx.Ins)
				expect.ProduceUTXOs(t, s, utx.Outs, txID, 0)
				return s
		...
		"Before Cairo/OK: unlock available amount, deposit is still unlocking": {
			state: func(t *testing.T, c *gomock.Controller, phase test.Phase, cfg *config.Config, utx *txs.UnlockDepositTx, txID ids.ID) *state.MockDiff {
				s := state.NewMockDiff(c)
				// checks
				s.EXPECT().CaminoConfig().Return(&state.CaminoConfig{LockModeBondDeposit: true}, nil)
				...
				s.EXPECT().GetTimestamp().Return(chainTime).Times(2)
				s.EXPECT().GetBaseFee().Return(test.TxFee, nil)
				expect.VerifyUnlockDeposit(t, s, utx.Ins,
				...
				s.EXPECT().ModifyDeposit(depositTxID1, &deposits.Deposit{
					...
					UnlockedAmount:      deposit1.UnlockedAmount + depositAmount1/2,
			...
			utx: func(phase test.Phase, cfg *config.Config) *txs.UnlockDepositTx {
				phaseTime := test.PhaseTime(t, phase, cfg)
				...
				// deposit1 halfUnlockTime1, see tt.state
				chainTime := deposit1.StartUnlockTime(offer).Add(offer.UnlockPeriodDurationNano() / 2)
				...
				return &txs.UnlockDepositTx{BaseTx: txs.BaseTx{BaseTx: avax.BaseTx{
					...
					Outs: []*avax.TransferableOutput{
						...
						generate.Out(test.AVAXAssetID, depositAmount2-unlockedDepositAmount2, owner1, depositTxID2, ids.Empty),
					},
				}}}
			},
		...
		"After Cairo/Wrong lockModeBondDeposit flag": {
			state: func(_ *testing.T, c *gomock.Controller, _ test.Phase, _ *config.Config, utx *txs.UnlockDepositTx, txID ids.ID) *state.MockDiff {
				s := state.NewMockDiff(c)
				s.EXPECT().CaminoConfig().Return(&state.CaminoConfig{LockModeBondDeposit: false}, nil)
				return s
		...
		"After Cairo/Unlock before deposit's unlock period": {
			state: func(t *testing.T, c *gomock.Controller, phase test.Phase, cfg *config.Config, utx *txs.UnlockDepositTx, txID ids.ID) *state.MockDiff {
				s := state.NewMockDiff(c)
				s.EXPECT().CaminoConfig().Return(&state.CaminoConfig{LockModeBondDeposit: true}, nil)
				phaseTime := test.PhaseTime(t, phase, cfg)
				...
				s.EXPECT().GetTimestamp().Return(deposit.StartUnlockTime(offer).Add(-time.Second))
				s.EXPECT().GetBaseFee().Return(test.TxFee, nil)
				expect.VerifyUnlockDeposit(t, s, utx.Ins,
					[]*avax.UTXO{feeUTXO, deposit1UTXO},
					[]ids.ShortID{
		...
		"After Cairo/Unlock more, than available": {
			state: func(t *testing.T, c *gomock.Controller, phase test.Phase, cfg *config.Config, utx *txs.UnlockDepositTx, txID ids.ID) *state.MockDiff {
				s := state.NewMockDiff(c)
				s.EXPECT().CaminoConfig().Return(&state.CaminoConfig{LockModeBondDeposit: true}, nil)
				phaseTime := test.PhaseTime(t, phase, cfg)
		...
		"After Cairo/Only burn fee, nothing unlocked": {
			state: func(t *testing.T, c *gomock.Controller, phase test.Phase, cfg *config.Config, utx *txs.UnlockDepositTx, txID ids.ID) *state.MockDiff {
				s := state.NewMockDiff(c)
				s.EXPECT().CaminoConfig().Return(&state.CaminoConfig{LockModeBondDeposit: true}, nil)
				phaseTime := test.PhaseTime(t, phase, cfg)
		...
		"After Cairo/No deposited input": {
			state: func(t *testing.T, c *gomock.Controller, phase test.Phase, cfg *config.Config, utx *txs.UnlockDepositTx, txID ids.ID) *state.MockDiff {
				s := state.NewMockDiff(c)
				// checks
				s.EXPECT().CaminoConfig().Return(&state.CaminoConfig{LockModeBondDeposit: true}, nil)
		...
		"After Cairo/Expired deposit": {
			state: func(t *testing.T, c *gomock.Controller, phase test.Phase, cfg *config.Config, utx *txs.UnlockDepositTx, txID ids.ID) *state.MockDiff {
				s := state.NewMockDiff(c)
				// checks
				s.EXPECT().CaminoConfig().Return(&state.CaminoConfig{LockModeBondDeposit: true}, nil)
				phaseTime := test.PhaseTime(t, phase, cfg)
				s.EXPECT().GetTimestamp().Return(phaseTime)
				s.EXPECT().GetBaseFee().Return(test.TxFee, nil)
				expect.VerifyUnlockDeposit(t, s, utx.Ins,
					[]*avax.UTXO{feeUTXO, deposit1UTXO},
					[]ids.ShortID{
				...
				s.EXPECT().GetDeposit(depositTxID1).Return(&deposits.Deposit{}, nil) // expired deposit
				return s
			},
			utx: func(p test.Phase, c *config.Config) *txs.UnlockDepositTx {
				return &txs.UnlockDepositTx{BaseTx: txs.BaseTx{BaseTx: avax.BaseTx{
			...
			signers:     [][]*secp256k1.PrivateKey{{feeOwnerKey}, {owner1Key}},
			phase:       test.PhaseLast,
			expectedErr: errDepositExpired,
		},
		"After Cairo/OK: unlock available amount": {
			state: func(t *testing.T, c *gomock.Controller, phase test.Phase, cfg *config.Config, utx *txs.UnlockDepositTx, txID ids.ID) *state.MockDiff {
				s := state.NewMockDiff(c)
				// checks
				s.EXPECT().CaminoConfig().Return(&state.CaminoConfig{LockModeBondDeposit: true}, nil)
	...
	for name, tt := range tests {
		t.Run(name, func(t *testing.T) {
			...
			cfg := test.Config(t, tt.phase)
			utx := tt.utx(tt.phase, cfg)
			utx.BlockchainID = backend.Ctx.ChainID
			utx.NetworkID = backend.Ctx.NetworkID
			tx, err := txs.NewSigned(utx, txs.Codec, tt.signers)
			require.NoError(err)
			...
			err = tx.Unsigned.Visit(&CaminoStandardTxExecutor{
				StandardTxExecutor{
					Backend: backend,
					State:   tt.state(t, gomock.NewController(t), tt.phase, cfg, utx, tx.ID()),
					Tx:      tx,
				},
			})
...
func TestCaminoStandardTxExecutorClaimTx(t *testing.T) {
	...
	msigFeeUTXO := generate.UTXO(ids.GenerateTestID(), test.AVAXAssetID, test.TxFee, *feeMsigOwner, ids.Empty, ids.Empty, true)
...
func TestCaminoStandardTxExecutorRewardsImportTx(t *testing.T) {
	...
	shmWithUTXOs := func(t *testing.T, c *gomock.Controller, utxos []*avax.TimedUTXO) *atomic.MockSharedMemory {
		...
		shm.EXPECT().Indexed(test.CChainID, treasury.AddrTraitsBytes,
	...
	tests := map[string]struct {
		...
		"Imported inputs don't contain all reward utxos that are ready to be imported": {
			...
			utxos: []*avax.TimedUTXO{
				{
					UTXO:      *generate.UTXO(ids.ID{1}, test.AVAXAssetID, 1, *treasury.Owner, ids.Empty, ids.Empty, true),
					Timestamp: athensBlockTimestamp - atomic.SharedMemorySyncBound,
				},
				{
					UTXO:      *generate.UTXO(ids.ID{2}, test.AVAXAssetID, 1, *treasury.Owner, ids.Empty, ids.Empty, true),
					Timestamp: athensBlockTimestamp - atomic.SharedMemorySyncBound,
				},
				{
					UTXO:      *generate.UTXO(ids.ID{3}, test.AVAXAssetID, 1, *treasury.Owner, ids.Empty, ids.Empty, true),
					Timestamp: athensBlockTimestamp - atomic.SharedMemorySyncBound,
				},
			},
		...
		"Imported input doesn't match reward utxo": {
			...
			utx: func(utxos []*avax.TimedUTXO) *txs.RewardsImportTx {
				return &txs.RewardsImportTx{BaseTx: txs.BaseTx{BaseTx: avax.BaseTx{
					...
					BlockchainID: constants.PlatformChainID,
					Ins: []*avax.TransferableInput{
						generate.In(test.AVAXAssetID, 1, ids.Empty, ids.Empty, []uint32{}),
					},
				}}}
			},
			utxos: []*avax.TimedUTXO{{
		...
		"OK: after AthensPhase": {
			...
			utxos: []*avax.TimedUTXO{
				{ // timed utxo, old enough
					UTXO:      *generate.UTXO(ids.ID{1}, test.AVAXAssetID, 3, *treasury.Owner, ids.Empty, ids.Empty, true),
					Timestamp: athensBlockTimestamp - atomic.SharedMemorySyncBound,
				},
				{ // not timed utxo
					UTXO: *generate.UTXO(ids.ID{2}, test.AVAXAssetID, 5, *treasury.Owner, ids.Empty, ids.Empty, true),
				},
				{ // timed utxo, old enough
					UTXO:      *generate.UTXO(ids.ID{3}, test.AVAXAssetID, 2, *treasury.Owner, ids.Empty, ids.Empty, true),
					Timestamp: athensBlockTimestamp - atomic.SharedMemorySyncBound,
				},
				{ // timed utxo, not old enough
					UTXO:      *generate.UTXO(ids.ID{4}, test.AVAXAssetID, 1, *treasury.Owner, ids.Empty, ids.Empty, true),
			...
			expectedAtomicRequests: func(utxos []*avax.TimedUTXO) map[ids.ID]*atomic.Requests {
				...
				return map[ids.ID]*atomic.Requests{test.CChainID: {
...
func TestCaminoCrossExport(t *testing.T) {
	...
	tests := map[string]struct {
		...
		"CrossTransferOutput OK": {
			utxos: []*avax.UTXO{
				generate.UTXO(ids.ID{0}, test.AVAXAssetID, test.ValidatorWeight, outputOwners, ids.Empty, ids.Empty, true),
			},
			ins: []*avax.TransferableInput{
				generate.InWithTxID(ids.ID{0}, test.AVAXAssetID, test.ValidatorWeight, ids.Empty, ids.Empty, sigIndices),
			},
			exportedOuts: []*avax.TransferableOutput{
				generate.CrossOut(test.AVAXAssetID, test.ValidatorWeight-test.TxFee, outputOwners, addr1),
			},
		},
		"CrossTransferOutput Invalid Recipient": {
			...
			exportedOuts: []*avax.TransferableOutput{
				generate.CrossOut(test.AVAXAssetID, test.ValidatorWeight-test.TxFee, outputOwners, ids.ShortEmpty),
			},
...
func TestCaminoStandardTxExecutorMultisigAliasTx(t *testing.T) {
	...
	ownerUTXO := generate.UTXO(ids.ID{1}, test.AVAXAssetID, test.TxFee, owner, ids.Empty, ids.Empty, true)
	msigUTXO := generate.UTXO(ids.ID{3}, test.AVAXAssetID, test.TxFee, *msigOwner, ids.Empty, ids.Empty, true)
...
func TestCaminoStandardTxExecutorAddProposalTx(t *testing.T) {
	...
	feeUTXO := generate.UTXO(ids.ID{1, 2, 3, 4, 5}, test.AVAXAssetID, test.TxFee, feeOwner, ids.Empty, ids.Empty, true)
	bondUTXO := generate.UTXO(ids.ID{1, 2, 3, 4, 6}, test.AVAXAssetID, proposalBondAmt, bondOwner, ids.Empty, ids.Empty, true)
	...
	baseTxWithBondAmt := func(bondAmt uint64) *txs.BaseTx {
		return &txs.BaseTx{BaseTx: avax.BaseTx{
			...
			Outs: []*avax.TransferableOutput{
				generate.Out(test.AVAXAssetID, bondAmt, bondOwner, ids.Empty, locked.ThisTxID),
			},
		}}
	}
...
func TestCaminoStandardTxExecutorFinishProposalsTx(t *testing.T) {
	...
	earlySuccessfulProposalUTXO := generate.UTXOWithIndex(earlySuccessfulProposalID, 0, test.AVAXAssetID, proposalBondAmt, bondOwner1, ids.Empty, earlySuccessfulProposalID, true)
	earlyFailedProposalUTXO := generate.UTXOWithIndex(earlyFailedProposalID, 0, test.AVAXAssetID, proposalBondAmt, bondOwner2, ids.Empty, earlyFailedProposalID, true)
	expiredSuccessfulProposalUTXO := generate.UTXOWithIndex(expiredSuccessfulProposalID, 0, test.AVAXAssetID, proposalBondAmt, bondOwner3, ids.Empty, expiredSuccessfulProposalID, true)
	expiredFailedProposalUTXO := generate.UTXOWithIndex(expiredFailedProposalID, 0, test.AVAXAssetID, proposalBondAmt, bondOwner4, ids.Empty, expiredFailedProposalID, true)
	activeSuccessfulProposalUTXO := generate.UTXOWithIndex(activeSuccessfulProposalID, 0, test.AVAXAssetID, proposalBondAmt, bondOwner5, ids.Empty, activeSuccessfulProposalID, true)
	activeFailedProposalUTXO := generate.UTXOWithIndex(activeFailedProposalID, 0, test.AVAXAssetID, proposalBondAmt, bondOwner6, ids.Empty, activeFailedProposalID, true)
	earlySuccessfulProposalWithBondUTXO := generate.UTXOWithIndex(earlySuccessfulProposalWithBondID, 0, test.AVAXAssetID, proposalBondAmt, bondOwner7, ids.Empty, earlySuccessfulProposalWithBondID, true)
	expiredSuccessfulProposalWithBondUTXO := generate.UTXOWithIndex(expiredSuccessfulProposalWithBondID, 0, test.AVAXAssetID, proposalBondAmt, bondOwner8, ids.Empty, expiredSuccessfulProposalWithBondID, true)
	additionalBondUTXO1 := generate.UTXOWithIndex(validatorTxID1, 0, test.AVAXAssetID, proposalBondAmt, bondOwner9, ids.Empty, validatorTxID1, true)
	additionalBondUTXO2 := generate.UTXOWithIndex(validatorTxID2, 0, test.AVAXAssetID, proposalBondAmt, bondOwner10, ids.Empty, validatorTxID2, true)
			...
			utx: func(cfg *config.Config) *txs.FinishProposalsTx {
				return &txs.FinishProposalsTx{
					BaseTx: txs.BaseTx{BaseTx: avax.BaseTx{
						...
						Outs: []*avax.TransferableOutput{
							generate.OutFromUTXO(t, earlySuccessfulProposalUTXO, ids.Empty, ids.Empty),
							generate.Out(test.AVAXAssetID, proposalBondAmt, bondOwner1, ids.Empty, ids.Empty), // expiredFailedProposalUTXO with different owner
						},
					}},
					EarlyFinishedFailedProposalIDs: []ids.ID{earlyFailedProposalID},
					ExpiredFailedProposalIDs:       []ids.ID{expiredFailedProposalID},
