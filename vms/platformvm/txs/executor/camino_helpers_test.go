...
func newExecutorBackend(
	...
	uptimes := uptime.NewManager(state, clk)
	utxoHandler := utxo.NewCaminoHandler(ctx, clk, fx, true)
	...
	t.Cleanup(func() {
		...
		if isBootstrapped.Get() {
			validatorIDs := config.Validators.GetValidatorIDs(constants.PrimaryNetworkID)
			require.NoError(t, uptimes.StopTracking(validatorIDs, constants.PrimaryNetworkID))
			for subnetID := range config.TrackedSubnets {
				validatorIDs := config.Validators.GetValidatorIDs(subnetID)
				require.NoError(t, uptimes.StopTracking(validatorIDs, subnetID))
			}
			state.SetHeight(math.MaxUint64)
