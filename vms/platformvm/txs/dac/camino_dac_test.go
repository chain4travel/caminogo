...
func TestProposalVerifierBaseFeeProposal(t *testing.T) {
	...
	for name, tt := range tests {
		t.Run(name, func(t *testing.T) {
			...
			require.NoError(t, err)
			err = proposal.Visit(ProposalVerifier(tt.state(ctrl, utx), fx, tx, utx))
...
func TestProposalExecutorBaseFeeProposal(t *testing.T) {
	...
	for name, tt := range tests {
		t.Run(name, func(t *testing.T) {
			...
			defer ctrl.Finish()
			fx := defaultFx(true)
			err := tt.proposal.Visit(ProposalExecutor(tt.state(ctrl), fx))
			require.ErrorIs(t, err, tt.expectedErr)
		})
	}
}
...
func TestProposalBondTxIDsGetterExcludeMemberProposal(t *testing.T) {
	...
	for name, tt := range tests {
		t.Run(name, func(t *testing.T) {
			ctrl := gomock.NewController(t)
			defer ctrl.Finish()
			bondTxIDs, err := tt.proposal.GetBondTxIDs(ProposalBondTxIDsGetter(tt.state(ctrl, tt.proposal)))
			require.ErrorIs(t, err, tt.expectedErr)
