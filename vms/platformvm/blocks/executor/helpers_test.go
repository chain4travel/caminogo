...
func newEnvironment(t *testing.T, ctrl *gomock.Controller) *environment {
	...
	if ctrl == nil {
		...
		res.utxosHandler = utxo.NewHandler(res.ctx, res.config, res.clk, res.state, res.fx)
		...
		res.utxosHandler = utxo.NewHandler(res.ctx, res.config, res.clk, res.mockedState, res.fx)
		res.txBuilder = p_tx_builder.New(
			res.ctx,
			res.config,
			res.clk,
