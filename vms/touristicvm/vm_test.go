...
func TestGenesis(t *testing.T) {
	...
	genesisBlock, err := vm.GetBlock(nil, lastAccepted) // genesisBlock as snowman.Block
	require.NoError(err)
...
func TestHappyPath(t *testing.T) {
	...
	require.NoError(err)
	genesisBlock, err := vm.GetBlock(nil, lastAcceptedID)
	require.NoError(err)
	...
	block2, err := vm.GetBlock(nil, lastAcceptedID)
	require.NoError(err)
	...
	block3, err := vm.GetBlock(nil, lastAcceptedID)
	require.NoError(err)
	...
	block2FromState, err := vm.GetBlock(nil, block2.ID())
	require.NoError(err)
	require.Equal(block2.ID(), block2FromState.ID())

	block3FromState, err := vm.GetBlock(nil, snowmanBlock3.ID())
	require.NoError(err)
