...
var (
	customGenesisConfigJSON = `{
		...
		"initialMultisigAddresses": [{"alias":"6ZmBHXTqjknJoZtXbnJ6x7af863rXDTwx", "addresses": ["NF3dhwiiGHc1MoT85T7MwWk2xLF9zpgeh", "7km6DDHSnzZZdSJx32tnZ8TDpjXNXSgJA"], "threshold":2}],
...
func TestGenesisFromFile(t *testing.T) {
	tests := map[string]struct {
		...
		err                     string
		expected                string
		expectedGenesisProperty interface{}
	}{
		"camino": {
		...
		"custom": {
			networkID:    9999,
			customConfig: customGenesisConfigJSON,
			expected:     "7c2173b562b6aab08dfb245fbcd804e39bab3c80b678c212bed2511878032496",
			expectedGenesisProperty: []*platformvm.GenesisMultisigAlias{&platformvm.GenesisMultisigAlias{
				Alias: func() ids.ShortID { s, _ := ids.ShortFromString("6ZmBHXTqjknJoZtXbnJ6x7af863rXDTwx"); return s }(),
				Owners: secp256k1fx.OutputOwners{
					Addrs: []ids.ShortID{
						func() ids.ShortID { s, _ := ids.ShortFromString("NF3dhwiiGHc1MoT85T7MwWk2xLF9zpgeh"); return s }(),
	...
	for name, test := range tests {
		t.Run(name, func(t *testing.T) {
			...
			if test.expectedGenesisProperty != nil {
				ids.SortShortIDs(test.expectedGenesisProperty.([]*platformvm.GenesisMultisigAlias)[0].Owners.Addrs)
				assert.Equal(test.expectedGenesisProperty, genesis.MultisigAliases)
			}
			assert.NoError(err)
		})
	}
...
func TestGenesisFromFlag(t *testing.T) {
	tests := map[string]struct {
		...
		"custom": {
			networkID:    9999,
			customConfig: customGenesisConfigJSON,
			expected:     "25bbcf08e56b82fbd3b8dec9700540d72945dc7387be0fd2d7b7920f81e03429",
		},
		"custom (networkID mismatch)": {
			networkID:    9999,
			customConfig: localGenesisConfigJSON,
